...
while true; do
	...
	saved_networks_with_info() {
		nmcli -f "TIMESTAMP,NAME,TYPE,DEVICE" c show |
			sed '1d' |
			awk '!/(lo|docker0|tun0)/' |
			sort -u -r |
			grep -E -e "wifi"
	...
	saved_network () {
		saved_networks_with_info | awk '{print $2}'
	}
	...
	*"disabled"*)
	...
	"Manual Entry")
		...
		;;
	"Saved Networks")
		selected_option=$(echo "Wi-Fi"$'\n'"$(saved_network)" | rofi -dmenu -i -selected-row 1 -config "${config}" -p " " || pkill -x rofi)

		if [ -z selected_option ]; then 
			exit
		fi
		if nmcli con up "$selected_option" | grep -q "successfully"; then
			notify-send "Connected to $selected_option."
			exit
		else
			notify-send "Failed to connect to $selected_option."
		fi
		;;
	*)
